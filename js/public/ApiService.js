    ...
    //格隆汇ajax
    var glhAjax = {};
    glhAjax.get = function(sets)
    {
      var success = sets.success || function(){};
      sets.targets = sets.targets || $(document);
      sets.evt = sets.evt || 'now';
      $.fn.api.settings.api=  { posts:sets.url};
      var listObj =
      {
      action: 'posts',
      method: 'get',
      on:sets.evt,
      beforeSend:sets.beforeSend || function(settings){ return {};},
      onSuccess : function(response){
        success(response);
      },
      };
      sets.targets.api(listObj);
      console.log(sets.query);
      if(sets.query){
          sets.targets.api('query');
      }
    }
